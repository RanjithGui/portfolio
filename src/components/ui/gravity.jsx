...
export const Gravity = forwardRef(
  (
    ...
    const updateElements = useCallback(() => {
      ...
      // Only keep the loop alive while the engine runner is active
    ...
    const initializeRenderer = useCallback(() => {
      ...
      // Point the mouse at our container div (not the canvas) so we can
      // strip wheel listeners without losing drag support
      const mouse = Mouse.create(canvas.current)

      // Matter.js adds mousewheel/DOMMouseScroll listeners that call
      // preventDefault — this blocks page scrolling. Remove them so the
      // user can scroll normally while still being able to drag pills.
      mouse.element.removeEventListener("mousewheel", mouse.mousewheel)
      mouse.element.removeEventListener("DOMMouseScroll", mouse.mousewheel)

      // The injected <canvas> sits on top of the DOM; make it transparent
      // to pointer events so clicks/scroll fall through to the page.
      ...
      // Thick walls (200px) so fast-thrown pills can't tunnel through
      const T = 200
      const walls = [
      ...
      // Cap velocity after every step so pills can't escape at extreme speed
      const MAX_SPEED = 25
      Events.on(engine.current, "afterUpdate", () => {
        engine.current.world.bodies.forEach((body) => {
          if (!body.isStatic && body.speed > MAX_SPEED) {
            Body.setVelocity(body, {
              x: body.velocity.x * (MAX_SPEED / body.speed),
              y: body.velocity.y * (MAX_SPEED / body.speed),
            })
          }
        })
      })
      ...
      // Prevent page scroll (wheel / trackpad) while actively dragging a pill
      const onWheel = (e) => { if (mouseDown.current) e.preventDefault() }
      canvas.current.addEventListener("wheel", onWheel, { passive: false })

      // Strip Matter.js touch listeners — they call preventDefault on touchmove
      // which blocks finger-scrolling. We re-add them only during active drag.
